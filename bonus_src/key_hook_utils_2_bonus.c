...
static bool	check_diags(t_data *data, double new_x, double new_y)
{
	...
	if (ft_strchr("D1", map->text[diag_y][(int) data->pos_x]) != NULL)
		return (false);
	if (ft_strchr("D1", map->text[(int) data->pos_y][diag_x]) != NULL)
		return (false);
	return (true);
}
...
bool	wall_collision(t_data *data, double new_pos_x, double new_pos_y)
{
	...
	if (ft_strchr("D1", map->text[(int) new_pos_y][(int) new_pos_x]) != NULL)
		return (true);
	if (ft_strchr("D1", map->text[(int) adjusted_y][(int) adjusted_x]) != NULL)
		return (true);
